...
public class EnemyPatrolA1 : MonoBehaviour
{
    ...
    {
        StopAllCoroutines();
        _randomNumber = (int)Random.Range(0, _referenceToMoveA.Length);
    ...
    IEnumerator Patrol()
    {
        ...
        while (true)
        {
            transform.position = Vector3.MoveTowards(transform.position, _referenceToMoveA[_randomNumber].transform.position, 0.05f);
            transform.LookAt(_referenceToMoveA[_randomNumber].transform.position);
            if (Vector3.Distance(transform.position, _referenceToMoveA[_randomNumber].transform.position) < 0.1f)
            {
                transform.position = _referenceToMoveA[_randomNumber].transform.position;
